...
# Metavaults
VAULTS: list[ChecksumAddress] = config(
    'VAULTS', cast=lambda x: [Web3.to_checksum_address(vault) for vault in x.split(',')]
)

MIN_ACTIVATION_BALANCE = Web3.to_wei(32, 'ether')


# Allow the user to set a custom minimum deposit amount in ETH (GNO), convert it to Wei
METAVAULT_MIN_DEPOSIT_AMOUNT: Wei = config(
    'METAVAULT_MIN_DEPOSIT_AMOUNT',
